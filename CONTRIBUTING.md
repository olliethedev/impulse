...
On a Linux desktop with Python 3 and Docker or rootless Podman, `bash scripts/install-local.sh` opts this checkout into a managed development install after validation. It builds with the pinned container, atomically replaces `~/.local/bin/impulse`, refreshes the selected user skill, and restarts dispatch if it was running. The ignored `.impulse-install.json` records ownership and the installed binary checksum. `IMPULSE_INSTALL_BIN`, `IMPULSE_INSTALL_HARNESS`, and `CONTAINER_ENGINE` support other local choices. Existing work is not an installation gate; the developer chooses when to run it. Native macOS/Windows contributors build and install their native binary separately.

Run `python3 tests/test-install-local.py` on the Linux host for isolated installer failure checks; it uses fixture executables and does not touch the installed CLI or scheduler. The Bun devcontainer intentionally covers the portable CLI, while this host test covers the Linux development installer.
