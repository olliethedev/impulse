...
- Codex 0.153.2 now runs the authorized local recovery assignment through a private app-server socket in Yakuake. Its first-use project-trust prompt was observed, that owned run was gracefully cancelled, and recovery relaunched after project setup. No indexing submissions are allowed during this initial recovery.
...
- [Migration CI run 33995048096](https://github.com/olliethedev/impulse/actions/runs/33995048096) passed at `c99e8ea` for all five native targets and the devcontainer, including the Linux installer failure checks. The suite now has 25 cases: POSIX runs 24 and skips the Windows-specific batch case; Windows runs 23 and skips the two POSIX terminal/process-group cases. The Windows case verifies a batch file path with spaces and a quoted ampersand argument. No hosted job invokes a real model.
